...
def create_pairs(x, digit_indices):
    ...
    #n=len-1 for creating last image pair 5988+5989
    for d in range(10):
        for i in range(n):
            z1, z2 = digit_indices[d][i], digit_indices[d][i + 1]
            pairs += [[x[z1], x[z2]]]#creating similar image pair
            inc = random.randrange(1, 10)
            dn = (d + inc) % 10
            z1, z2 = digit_indices[d][i], digit_indices[dn][i]
            pairs += [[x[z1], x[z2]]]#Creating non-similar image pair
            labels += [1, 0]
...
def create_pairs_on_set(images, labels):
    ...
    digit_indices = [np.where(labels == i)[0] for i in range(10)]#Finding which image images has label i=0123...
